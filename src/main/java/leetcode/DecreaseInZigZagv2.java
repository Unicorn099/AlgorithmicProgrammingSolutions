...
public class DecreaseInZigZagv2 {

  public static void main(String[] args) {

    int[] arr = {10, 4, 4, 10, 10, 6, 2, 3};
    DecreaseInZigZagv2 d = new DecreaseInZigZagv2();
  ...
  public int movesToMakeZigzag(int[] nums) {
    ...
    if (nums[i] > nums[i - 1]) {
      ...
      c2 = new CurrentStatus(nums[i], Math.abs(nums[i] - (nums[i - 1] + 1)), 'S');
    } else {
  ...
  private CurrentStatus geUpdatedVal(CurrentStatus c1, int num) {
    ...
    if (c1.expectedEle == 'S') {
      if (num < c1.lastElement) {
        ...
        c1.expectedEle = 'G';
        int x = num;
        num = c1.lastElement-1;
        c1.lastElement = num;
        c1.moves = c1.moves + Math.abs(x - num);
      }
    } else {
      if (num > c1.lastElement) {
        c1.expectedEle = 'S';
